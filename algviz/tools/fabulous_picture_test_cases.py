...
def test_null_children_tree(test_dir, outfile):
    ...
    b = structures.String("b", uid=2)
    c = structures.String("c", uid=3)
    t1 = structures.TreeNode(b, [structures.Null], uid=4)
    t2 = structures.TreeNode(c, [structures.Null, structures.Null], uid=5)
    t3 = structures.TreeNode(a, [t1, structures.Null, t2], uid=6)
    pic = TreePicture(t3)
    pic.draw()
    pic.save(os.path.join(test_dir, outfile))
